...
describe('JF-Toppage Test', () => {
  ...
  it('2.1.1 Test display Jf Detail - Date', () => {
    cy.get('div[class^="admin__jf"]').find('h3').eq(0).invoke('text')
      .should('not.be.eq', '')
  })
  it('2.1.1 Test display Jf Detail - Admin', () => {
    cy.get('div[class^="admin__jf"]').find('h3').eq(1).invoke('text')
      .should('not.be.eq', '')
  })
  it('2.1.1 Test display Jf Detail - Company-number', () => {
    cy.get('div[class^="admin__jf"]').find('h3').eq(2).invoke('text')
      .should('not.be.eq', '')
  })
  it('2.1.1 Test display Jf Detail - Student-number', () => {
    cy.get('div[class^="admin__jf"]').find('h3').eq(3).invoke('text')
      .should('not.be.eq', '')
  })
  it('2.2.1 Test display Side-bar - Home', () => {
    cy.get('li[class^="ant-menu"]').find('a').eq(0).invoke('text')
      .should('be.eq', 'ホーム')
  })
  it('2.2.2 Test display Side-bar - Task', () => {
    cy.get('li[class^="ant-menu"]').find('a').eq(1).invoke('text')
      .should('be.eq', 'タスク')
  })
  it('2.2.3 Test display Side-bar - GantChart', () => {
    cy.get('li[class^="ant-menu"]').find('a').eq(2).invoke('text')
      .should('be.eq', 'ガントチャート')
  })
  it('2.2.4 Test display Side-bar - KanBan', () => {
    cy.get('li[class^="ant-menu"]').find('a').eq(3).invoke('text')
      .should('be.eq', 'カンバン')
  })
  it('2.2.5 Test display Side-bar - File', () => {
    cy.get('li[class^="ant-menu"]').find('a').eq(4).invoke('text')
      .should('be.eq', 'ファイル')
  })
  ...
  it('2.6 Test display Status Erea ', () => {
    cy.get('div[class^="status__global"]').eq(0).find('h3').invoke('text')
      .should('be.eq', 'ステータス')
  })
  it('2.7 Test display Milestone Erea ', () => {
    cy.get('div[class^="status__global"]').eq(1).find('h3').invoke('text')
  ...
  it('3.2.5 "Show more" button - after click "もっと見る"(30)', () => {
    cy.get('div[href*="task-list"]').then(($div) => {
      if ($div.length < 5) {
        cy.get('button').contains('もっと見る').should('have.length', 0)
        cy.get('button').contains('表示数を戻す').should('have.length', 0)
        ...
        cy.get('button').contains('もっと見る').click()
        cy.get('button').contains('表示数を戻す').should('have.length', 1)
        cy.get('div[href*="task-list"]').should('have.length', 30)
      }
    })
  })
