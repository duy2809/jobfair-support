...
describe.skip('Check button add file', () => {
    ...
    //     // })    
    //     cy.get('.ant-btn').find('span').contains('新しいファイル').click({ force: true })
    //     cy.get('input[placeholder*="新しいファイル名"]').type(initialData).should('have.value', initialData)
    //     cy.get('input[placeholder*="グーグルドライブリンク"]').type('asdf').should('have.value', 'asdf')
    //     cy.get('.ant-modal-footer').within(() => {             
    //         cy.get('.ant-btn').last().click().wait(1000)
    //     }) 
    ...
    //     cy.get('.ant-btn').find('span').contains('新しいファイル').click({ force: true })
    //     cy.get('input[placeholder*="新しいファイル名"]').type('HelloWorld111').should('have.value', 'HelloWorld111')
    //     cy.get('input[placeholder*="グーグルドライブリンク"]').type('asdf').should('have.value', 'asdf')
    //     cy.get('.ant-modal-footer').within(() => {
...
describe('Check button add folder', () => {
    it('Check modal add folder display', () => {
        ...
        cy.get('button').find('span').contains('新しいフォルダ').click()
        cy.get('.ant-modal-content').should('be.visible')
    ...
    it('Check input active', () => {
        cy.get('.ant-btn').find('span').contains('新しいフォルダ').click()
    ...
    // it('Check data existed', () => {
    //     let initialData = 'jobfair'
    //     // cy.contains('', '').parent().within((tr) => {
    //     //     cy.get('td:nth-child(2)').then(($span) => {
    //     //         initialData = $span.text()
    //     //     })
    //     // })
    //     cy.get('button').find('span').contains('新しいフォルダ').click()
    //     cy.get('input[placeholder*="新しいフォルダ名"]').type(initialData).should('have.value', initialData)
    //     cy.get('.ant-modal-footer').within(() => {
    ...
    //     cy.get('input').type('HelloWorld111')
    //     cy.get('.ant-modal-footer').within(() => {
    //         cy.get('.ant-btn').last().click({ force: true }).wait(1000)
    //     })
    //     cy.get('.ant-notification-notice-message').should('be.visible').should('contain', '新しいファイルを追加しました。')
...
describe('Check Search file', () => {
    ...
    // it('Check modal', () => {
    //     cy.get('.ant-form>.ant-row:nth-child(1) input').type('jobfair')
    //     cy.get('.ant-form>.ant-row:nth-child(2) input').should('have.value')

    //     cy.get('.ant-form>.ant-row:nth-child(4) .FileSelectBox .ant-select-selection-item').should('','Sun Asterisk')
              
    //     cy.get('.ant-modal-content').should('be.visible')
    //     cy.get('.ant-modal-close-x').should('be.visible')
    //     cy.get('.ant-modal-title').should('be.visible').contains('ファイルを検索')
    ...
    //     cy.get('.ant-modal-body .ant-table-tbody > tr:nth-child(2) > td:nth-child(3)').contains('Sun Asterisk').should('be.visible')
    // })
})
